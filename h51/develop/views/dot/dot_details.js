...
require(['jquery', 'Vue', 'api', 'common', 'Loading', 'Swiper', 'async!BMap'], function($, Vue, api, common, Loading, Swiper) {
	$(function() {
			...
			var myVue = new Vue({
				...
				methods: {
					...
					getUrlParams: function() {
						...
						//是否有店铺图片，有拼接图片数据
						if (decodeURIComponent(common.getRequest().logImgArr) != '') {
							var imgArrs = decodeURIComponent(common.getRequest().logImgArr).split(',');
	
							if (imgArrs.length) {							
								this.imgData = [
									{
										imgurl: imgArrs[1].split('##')[1]
									},							
									{
										imgurl: imgArrs[2].split('##')[1]
									},							
									{
										imgurl: imgArrs[3].split('##')[1]
									}							
								];
							} else {
								this.imgData = [];
					...
					returnHome: function() {
						...
						location.href = './index.html'+ common.jsonToQueryString(params);		
					},
					//获取进入附近称的URL
					goNearMap:function(){
