...
public class IndexController {
	...
	public String goIndex(@RequestParam(name = "_p") String p, ModelMap map, UserWeightVo vo, WeChatUserVo weCharUser,
		...
		String weightStr = paraMap.get("weight");
		if(StringUtils.isEmpty(weightStr)) {
			weightStr = "0";
			LOG.warn("weight is null, p={}", p);
		}
		BigDecimal bweight = null;
		try{
			bweight = new BigDecimal(weightStr);
		}catch(Exception e){
			bweight = new BigDecimal(0);
			LOG.error("weightStr:{}",weightStr);
		...
		if (weChatUserVo != null && weChatUserVo.getHeadimgurl()!=null) {
			...
			if(imgData==null){
				imgData = getImgStr(domainImg+"/images/no.jpg");
			}
	...
	public ApiData<UserWeightVo> page(@RequestParam(name = "_p") String p, @RequestParam Integer type) {
		...
		WeChatUserVo weChatUserVo = userInfoService.getOpenId(openId, appId);
		ApiData<UserWeightVo> apiData = indexService.wuList(type, weChatUserVo.getUnionid());
		UserWeightVo vo = new UserWeightVo();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss");
		vo.setReceiverTime(sdf.format(new Date()));
		vo.setWeight(weight);
		List<UserWeightVo> list = null;
		// 加上当前体重信息
		if (apiData == null) {
			apiData = new ApiData<>();
			apiData.setTotal(1L);
			...
			apiData.setTotal(apiData.getTotal()+1L);
	...
	private String getImgStr(String img) {
		...
		try {
			...
			LOG.error(e.getMessage() + ", img=" + img, e);
		} finally {
