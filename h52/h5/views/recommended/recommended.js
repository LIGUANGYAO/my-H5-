        ...
        setTimeout(function() {
            new Vue({
                ...
                mounted: function() {
                   ...
                   var Type = sessionStorage.recommended;

                   this.teachEatThemes(Type);


                },
                methods: {
                    teachEatThemes:function(Type){
                        var _this = this;
                        ...
                        $.ajax({
                            type: 'GET',
                            url: api.teachEatThemes+'?'+$.param(data),
                            dataType: 'json',
                            async: true,
                            success:function(res){
                                ...
                                if(res.result.data){
                                    for (var i = 0; i < res.result.data.length; i++) {
                                        var obj = {};
                                            obj["mainName"] = res.result.data[i].name;
                                            obj["picPath"] = res.result.data[i].picRealPath,
                                            obj["id"] = res.result.data[i].id,
                                            _this.hotMainData.push(obj);
                                    }

                                    console.log(_this.hotMainData)
                                }
                            }
                    ...
                    linkToHot: function(index) {
                        var mainid = this.hotMainData[index].id;

                        if(typeof(Storage) !== "undefined"){
                            sessionStorage.mealType= mainid;
                        }
                        common.linkTo2('../recommendedinner/breakfast.html');
